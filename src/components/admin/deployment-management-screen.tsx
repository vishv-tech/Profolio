...
export function DeploymentManagementScreen({ deployments, stats, filters, query, selectedDeploymentId }: { deployments: PageResult<AdminDeployment>; stats: { totalDeployments: number; currentVersions: number; historicalVersions: number; rolledBack: number }; filters: { portfolios: { id: string; title: string; slug: string }[] }; query: DeploymentQuery; selectedDeploymentId: string }) {
  ...
  return <div className="admin-page">
    <PageHeading title="Deployment history" description="Review portfolio publication versions, themes, and status." />
    ...
    <section className="admin-card admin-manager">
      <div className="admin-manager__filters"><form action="/admin/deployments" className="admin-filter-form"><input name="search" type="search" defaultValue={query.search} placeholder="Search portfolio or deployment UUID" aria-label="Search deployments" /><select name="status" defaultValue={query.status} aria-label="Filter deployment status"><option value="all">All statuses</option><option value="current">Current</option><option value="historical">Historical</option><option value="rolled_back">Rolled back</option></select><select name="portfolioId" defaultValue={query.portfolioId} aria-label="Filter portfolio"><option value="">All portfolios</option>{filters.portfolios.map((portfolio) => <option key={portfolio.id} value={portfolio.id}>{portfolio.title || portfolio.slug}</option>)}</select><select name="date" defaultValue={query.dateRange} aria-label="Filter deployment date"><option value="7">Last 7 days</option><option value="30">Last 30 days</option><option value="all">All time</option></select><select name="order" defaultValue={query.order} aria-label="Sort deployments"><option value="newest">Newest</option><option value="oldest">Oldest</option></select><button className="admin-button admin-button--primary" type="submit">Apply</button></form><Link href="/admin/deployments">Clear filters</Link></div>
