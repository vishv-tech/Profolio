...
export default function SageTerminalTheme({ data, config }: PavniThemeProps) {
  const fullName = data.personal.fullName.trim() || "Untitled portfolio";
  const summarySlot = (
    <section className={styles.intro} id="summary">
      <div>
        <p className={styles.index}>PROFILE / NOTEBOOK</p>
  ...
  return (
    <main
      ...
      data-theme-layout="pavni-sage-terminal"
      data-theme-pack="pavni"
      style={frameStyle(config)}
    >
      <header className={styles.cover}>
        <div aria-hidden="true" className={styles.grid} />
        <div aria-hidden="true" className={styles.leaf} />
        <div aria-hidden="true" className={styles.ring} />
        <div className={styles.coverNav}>
          <span>STUDIO / 24</span><span>FIELD NOTES</span><span>PORTFOLIO</span>
        </div>
      ...
      <footer className={styles.footer}>
        <span>SAGE TERMINAL / 24</span><span>FIELD / NOTES / ARCHIVE</span>
      </footer>
    </main>
  );
}
